...
def parse_env_file(env_file: Path) -> dict[str, str]:
	...
	with env_file.open("r", encoding="utf-8") as file:
		for raw_line in file:
			...
			if not key:
				raise ValueError(f"Empty key in {env_file}: {raw_line.rstrip()}")
			...
			if value[0] in ("'", '"'):
				...
				if end >= len(value):
					raise ValueError(f"Unterminated quoted value in {env_file}: {raw_line.rstrip()}")

				parsed_value = value[1:end]
			else:
