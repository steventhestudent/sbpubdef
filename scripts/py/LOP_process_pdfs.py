...
ignore_categories = ["Forms", "Contacts", "827 Petitions to Access Juvenile Case Files", "Records Requests"]
# walk resource dir
ignored = []
for root, dirs, files in os.walk(SCRAPED_LOP_PAGE_RESOURCES):
    for filename in files:
        ...
        if category in ignore_categories: continue # not a procedure
        if not(filename.endswith('.pdf')):
            if not(filename.endswith('.DS_Store')): ignored.append(f'{category}/{filename}')
            continue
        pc = ProcedureChecklist(filepath, category, LOCAL_OUT_DIR)
        pc.run()
        procedure_checklists.append(pc)
