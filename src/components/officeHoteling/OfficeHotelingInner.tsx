...
import type { IOfficeHotelingProps } from "../../webparts/officeHoteling/components/IOfficeHotelingProps";
import {
	...
	type StatusMessage,
	type TimeSlot,
} from "../../webparts/officeHoteling/components/officeHotelingUtils";
import {
...
export function OfficeHotelingInner(
	...
	const functionBase = String(ENV.FUNCTION_BASE_URL || "").replace(/\/$/, "");
	if (!functionBase) throw new Error("ENV.FUNCTION_BASE_URL is not set.");
	const SEND_EMAIL_FUNCTION_URL = `${functionBase}/api/SendEmail`;
	const SEND_EMAIL_API_APP_ID = ENV.FUNCTION_API_APP_ID;
	...
	const createSharePointEventForReservation = async (
		...
		const sp = getSharePointClient();
		const list = sp.web.lists.getByTitle(ENV.SP_CONTENTTYPE_EVENTS);
	...
	const deleteSharePointEventForReservation = async (
		...
		const sp = getSharePointClient();
		await sp.web.lists
			.getByTitle(ENV.SP_CONTENTTYPE_EVENTS)
