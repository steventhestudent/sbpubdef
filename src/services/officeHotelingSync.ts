...
const getCurrentUserKey = (): string => {
	...
	const rawUser =
		pageContext?.userEmail || pageContext?.userLoginName || "anonymous";

	return rawUser.toLowerCase().replace(/[^a-z0-9@._-]/g, "");
};
...
const ensureDebugSessionBaseline = (): void => {
	...
	try {
		...
		console.warn(
			"Failed to initialize hoteling debug session baseline.",
			error,
		);
	}
};
		...
		return parsed.filter((item): item is OfficeHotelingReservation => {
			...
			return (
				...
				(reservation.time === "Morning" ||
					reservation.time === "Afternoon") &&
				(reservation.outlookEventId === undefined ||
					typeof reservation.outlookEventId === "string") &&
				(reservation.outlookEventWebLink === undefined ||
					typeof reservation.outlookEventWebLink === "string") &&
				(reservation.sharePointEventId === undefined ||
					typeof reservation.sharePointEventId === "number") &&
				(reservation.sharePointEventWebLink === undefined ||
					typeof reservation.sharePointEventWebLink === "string")
			);
		});
	} catch (error) {
		console.warn(
			"Failed to read hoteling reservations from storage.",
			error,
		);
		return [];
	}
...
export const writeHotelingReservations = (
	...
	try {
		...
		console.warn(
			"Failed to write hoteling reservations to storage.",
			error,
		);
	}
};
...
export const clearHotelingReservations = (): void => {
	...
	try {
		...
		console.warn(
			"Failed to clear hoteling reservations from storage.",
			error,
		);
	}
};
