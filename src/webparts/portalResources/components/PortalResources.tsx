...
import {
	...
	IProcedureStep,
	IProcedure,
} from "@data/LOPProcedureChecklist.mock";
...
function ProcedureChecklist({
	...
	const [selectedProcedure, setSelectedProcedure] =
		React.useState<IProcedure | null>(null);
	...
	const handleSelectProcedure: (procedure: IProcedure) => void = (
		...
		<section className="p-4 text-sm">
			...
			<p className="mt-2 text-xs text-slate-500">
				{isLoading
					? "Loading procedures…"
					: selectedProcedure
						? `Viewing: ${selectedProcedure.title} - Step ${currentStep} of ${selectedProcedure.totalSteps}`
				...
				<>
					...
					<div className="mt-4 rounded-md bg-blue-50 border border-blue-200 p-3">
						...
						<div className="text-xs text-blue-600 space-y-1">
							...
							<p>
								• Actual screenshots will replace placeholders
							</p>
...
export function PortalResources(props: IPortalResourcesProps): JSX.Element {
	return (
		<Collapsible instanceId={props.context.instanceId} title="Resources">
			<PDRoleBasedSelect
				ctx={props.context}
				views={{
