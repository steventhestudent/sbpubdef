...
export function CardCarousel(props: ICardCarouselProps): JSX.Element {
  ...
  const [autoDetectedVertical, setAutoDetectedVertical] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      for (const entry of entries) {
        setAutoDetectedVertical(entry.contentRect.width < VERTICAL_BREAKPOINT);
      }
    });
    ro.observe(el);
  ...
  const isVertical = carouselMode === "vertical" || (carouselMode === "auto" && autoDetectedVertical);
  ...
  const effectiveBuffer = items.length <= effectiveVisible ? 0 : Math.min(BUFFER, Math.floor(extraCards / 2));
  ...
  const shiftCalc =
    slideShift !== 0 ? ` - ${slideShift} * (${cardWidthCalc} + ${GAP}px)` : "";
  ...
  function renderCard(item: ICardItem, key: string, flexStyle: React.CSSProperties): JSX.Element {
    return (
      <button
        key={key}
        type="button"
  ...
  if (isVertical) {
    ...
    return (
      <div ref={outerRef} style={{ width: "100%", maxWidth: "100%", overflow: "hidden", paddingBottom: 12, containerType: "inline-size" }}>
        <div
        ...
        >
          ...
          <div style={{ overflow: "hidden", height: vViewportHeightCalc }}>
            <div
              style={{
                display: "flex",
                flexDirection: "column",
                ...
                transition:
                  animateShift
                    ...
                    : "none",
                willChange: slideShift !== 0 ? "transform" : undefined,
              }}
            >
              {windowedItems.map((item, i) =>
                renderCard(item, `${item.key}__w${i}`, { flex: `0 0 ${vCardHeightCalc}`, width: "100%" })
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div ref={outerRef} style={{ width: "100%", maxWidth: "100%", overflowX: "hidden", paddingBottom: 12 }}>
      <div
        onMouseEnter={() => setIsPaused(true)}
        onMouseLeave={() => setIsPaused(false)}
      >
        ...
        <div style={{ overflow: "hidden" }}>
          ...
          >
            {windowedItems.map((item, i) =>
              renderCard(item, `${item.key}__w${i}`, { flex: `0 0 ${cardWidthCalc}` })
            )}
          </div>
        </div>
