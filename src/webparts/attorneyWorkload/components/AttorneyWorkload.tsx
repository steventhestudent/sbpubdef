...
import type { IAttorneyWorkloadProps, IAttorney } from './IAttorneyWorkloadProps';
...
export default function AttorneyWorkload(props: IAttorneyWorkloadProps): JSX.Element {
    ...
    locations.forEach(location => {
      ...
      location.caseTypes.forEach(ct => {
        ...
        ct.attorneys.forEach(att => {
          ...
          const caseMatches = att.cases.some(c =>
  ...
  const getAttorneyCaseCount = (att: IAttorney): number => getVisibleCases(att).length;

  const getCaseTypeCaseCount = (caseType: typeof locations[0]['caseTypes'][0]): number =>
    caseType.attorneys.reduce((total, att) => total + getVisibleCases(att).length, 0);

  const getLocationCaseCount = (location: typeof locations[0]): number =>
    location.caseTypes.reduce((total, ct) => total + getCaseTypeCaseCount(ct), 0);
          ...
          .map(ct => {
            ...
            const attorneys = ct.attorneys
              .map(att => {
                ...
                return attMatches || ctMatches || locationMatches ? { ...att } : null;
              })
              .filter((att): att is IAttorney => att !== null);

            return ctMatches || attorneys.length > 0 || locationMatches ? { ...ct, attorneys } : null;
          })
          .filter((ct): ct is typeof location.caseTypes[0] => ct !== null);

        return locationMatches || caseTypes.length > 0 ? { ...location, caseTypes } : null;
      })
      .filter((location): location is typeof locations[0] => location !== null);
  }, [locations, searchText]);

  return (
    <section className={styles.attorneyWorkload}>
      <Collapsible instanceId="attorney-workload" title="Attorney Workload">
            ...
            filteredLocations.map(location => {
              ...
              return (
                <div key={locationId} className={styles.locationCard}>
                  ...
                  >
                    <span>
                      {isLocationOpen ? '▼' : '▶'} {location.name}{' '}
                      <span className={styles.caseCount}>({getLocationCaseCount(location)})</span>
                    </span>
                  </button>
                      ...
                      return (
                        <div key={ctId} className={styles.caseTypeCard}>
                          ...
                          >
                            <span>
                              {isCtOpen ? '▼' : '▶'} {ct.type}{' '}
                              <span className={styles.caseCount}>({getCaseTypeCaseCount(ct)})</span>
                            </span>
                          </button>
                                  ...
                                  {isAttOpen && (
                                    <div className={styles.casesContainer}>
                                      {getVisibleCases(att).map(c => (
                                        <div key={c.CaseID} className={styles.caseNumber}>
                                          <strong>{c.CaseID}</strong> — {c.CaseStatus}
                                        </div>
                                      ))}
                                    </div>
                                  )}
