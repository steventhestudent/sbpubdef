		...
		return (
			<section className="rounded-xl border border-slate-200 bg-white shadow-sm">
				<header className="border-b border-slate-200 px-4 py-3">
					<h4 className="text-base font-semibold text-slate-800">Upcoming Events</h4>
				</header>
				<div className="overflow-x-auto">
					<table className="min-w-full divide-y divide-slate-200">
						<thead className="bg-slate-50">
						<tr>
							{['Date', 'Time', 'Event', 'Location', ''].map(h => (
						...
						{events.map((e, i) => (
							<tr key={i} className="hover:bg-slate-50">
								<td className="px-4 py-3 text-sm text-slate-800">{e.date}</td>
								<td className="px-4 py-3 text-sm text-slate-700">{e.time}</td>
								<td className="px-4 py-3 text-sm text-slate-800">{e.title}</td>
								<td className="px-4 py-3 text-sm text-slate-700">{e.where}</td>
				...
				<p className="px-4 py-3 text-xs text-slate-500"><em>Placeholder: Replace with calendar/events list data.</em></p>
			</section>
		);
	}
}
