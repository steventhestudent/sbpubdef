			...
			<div className="flex flex-wrap justify-start gap-3">
				{groupedReservations.map((group) => {
					...
					const reminderCooldown =
						reminderCooldownByReservationId[group.reservations[0].id] ?? 0;
						...
						>
							<div className="mb-4 flex-1 border-b border-slate-100 pb-3">
								<div className="flex items-start justify-between gap-3">
									<p className="text-slate-900 text-sm font-semibold leading-5">
								...
								<div className="space-y-1">
									{stationEntries.map((stationEntry, index) => (
										<p key={`${group.key}-station-${index}`} className="text-sm font-medium text-slate-700">
											{stationEntry}
										</p>
									))}
								</div>
...
interface AddReservationSectionProps {
	...
	handleNextWeek: () => void;
	timeSlots: TimeSlot[];
	isPendingSelection: (date: string, timeOfDay: "Morning" | "Afternoon") => boolean;
	handleSelectTimeSlot: (slot: TimeSlot, timeOfDay: "Morning" | "Afternoon") => void;
									...
									<button
										...
										onClick={() =>
											setOpenDropdown((current) =>
												current === "location" ? undefined : "location",
											)
										}
								...
								<div className="w-3/4">
									...
									<div className="relative" ref={stationDropdownRef}>
										<button
											...
											onClick={() =>
												setOpenDropdown((current) =>
													current === "station" ? undefined : "station",
												)
											}
											className={dropdownButtonClassName}
										>
					...
					<table className="w-full border-collapse table-fixed">
							...
							<tr>
								...
								{timeSlots.map((slot) => (
									...
									>
										...
										{slot.isHoliday && (
											<div className="mt-1 text-[10px] text-red-600">Holiday</div>
										)}
									</th>
								))}
							</tr>
						...
						<tbody>
							<tr>
								...
								{timeSlots.map((slot) => {
									const selected = isPendingSelection(slot.date, "Morning");
									return (
										<td key={`${slot.date}-morning`} className="border border-slate-300 p-2">
											<button
												...
												onClick={(e) => {
													e.preventDefault();
													e.stopPropagation();
													handleSelectTimeSlot(slot, "Morning");
												}}
							...
							<tr>
								...
								{timeSlots.map((slot) => {
									const selected = isPendingSelection(slot.date, "Afternoon");
									return (
										<td key={`${slot.date}-afternoon`} className="border border-slate-300 p-2">
											<button
												...
												onClick={(e) => {
													e.preventDefault();
													e.stopPropagation();
													handleSelectTimeSlot(slot, "Afternoon");
												}}
						...
						{summaryGroups.map((group) => {
							...
							return (
								...
								>
									...
									<div>
										<span className="font-medium">Station(s):</span>
										<div className="mt-1 space-y-0.5">
											{stationEntries.map((stationEntry, index) => (
												<p key={`${group.key}-confirm-station-${index}`}>
													{stationEntry}
												</p>
											))}
										</div>
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div className="bg-white rounded-md p-6 w-[90%] max-w-xl">
				...
				<p className="mb-4 text-slate-700">
					...
					{pendingDeleteIds.length > 1 ? " group" : ""}? This action cannot be undone.
				</p>
				<div className="flex gap-3 justify-end">
					<button
						type="button"
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div className="bg-white rounded-md p-6 w-[90%] max-w-xl">
				...
				<p className="mb-4 text-slate-700">
					You have reached the maximum of 3 active reservations. Please delete one
...
export function ReminderWaitModal({
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div className="bg-white rounded-md p-6 w-[90%] max-w-xl">
				<h4 className="font-semibold mb-3 text-blue-700">Please wait before sending again</h4>
				<p className="mb-4 text-slate-700">
